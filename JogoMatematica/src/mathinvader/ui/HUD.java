...
public class HUD {
    ...
    private void drawHearts(Graphics2D g, int lives, int x, int y) {
        // Draw hearts using ovals + triangle (removed redundant asterisk layer)
    ...
    private void drawInputBox(Graphics2D g, String input, int panelW, int panelH) {
        ...
        g.setFont(new Font("Courier New", Font.BOLD, fs.scale(11)));
        g.setColor(DIM);
        g.drawString("RESPOSTA:", bx + 12, by + 17);

        int answerSize = fs.scale(24);
        g.setFont(new Font("Courier New", Font.BOLD, answerSize));
