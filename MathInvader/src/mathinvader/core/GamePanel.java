...
public class GamePanel extends JPanel implements ActionListener {
    ...
    public enum GameState { MENU, PLAYING, PAUSED, GAMEOVER, AUDIO, RECORDES }
    ...
    private void handleMenuKey(KeyEvent e) {
        ...
        if (code == KeyEvent.VK_DOWN) menuSelection = Math.min(3, menuSelection + 1);
        if (code == KeyEvent.VK_ENTER) {
            if      (menuSelection == 0) startGame();
            else if (menuSelection == 1) state = GameState.RECORDES;
            else if (menuSelection == 2) state = GameState.AUDIO;
            else                         System.exit(0);
        }
    }
    ...
    private void spawnEnemy() {
        ...
        int maxEnemies = 3 + stats.getLevel();
        if (alive >= maxEnemies) return;

        int x = 80 + rand.nextInt(WIDTH - 160);
        Enemy enemy = new Enemy(x, WIDTH, stats.getLevel(), stats.getCurrentDifficulty());
    ...
    protected void paintComponent(Graphics g0) {
        ...
        switch (state) {
            ...
            case AUDIO:    background.draw(g); audioPanel.draw(g, WIDTH, HEIGHT); break;
    ...
    private void drawMenu(Graphics2D g) {
        ...
        String[] options = {"> JOGAR", "* RECORDES", ") AUDIO", "X SAIR"};
        ...
        g.drawString("Setas: NAVEGAR  |  ENTER: CONFIRMAR  |  M: MUSICA  |  S: SOM",
            cx - 200, HEIGHT - 32);
        g.setColor(new Color(60, 100, 80));
        g.drawString("[ F11 ] ou [ ALT+ENTER ] = Tela Cheia", cx - 140, HEIGHT - 14);
    }
